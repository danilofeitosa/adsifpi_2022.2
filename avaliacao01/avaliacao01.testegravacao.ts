...
class App {
    ...
    public carregarDeArquivo() {
        ...
        console.log("Inicializando a leitura do arquivo...")

        for (let i: number = 0; i < linhas.length; i++) {
            let linhaPerfil: string[] = linhas[i].split(";");
            
            let perfil!: Perfil;
            let id: number = linhaPerfil[0];
            let nome: string = linhaPerfil[1];
            let email: string = linhaPerfil[2];
        }
...
/*
function carregar_modelos_na_memoria(montadoras){
    const data = fs.readFileSync('modelos.txt', 'utf-8')
        ...
        for (let line of lines){
            ...
            if (modelo['turbo'] === "true"){
                modelo['turbo'] = "Sim"
            } else {
                modelo['turbo'] = "Não"
            }

            if (modelo['automatico'] === "true"){
                modelo['automatico'] = "Sim"
            } else {
                modelo['automatico'] = "Não"
            }

            for (let montadora of montadoras) {
                if (montadora['id'] === modelo['montadora']) {
...
function gravar_dados(montadoras){
    ...
    for (let m of montadoras){
        data_montadoras += `${m['id']}#${m['nome']}#${m['pais']}#${m['ano']}\n`
    }

    fs.writeFileSync('montadoras.txt', data_montadoras)

    let data_modelos = ''
    for (let m of montadoras){
        for (let n of m['modelos']) {
            if (n['turbo'] === 'Sim') {
                n['turbo'] = true
            } else {
                n['turbo'] = false
            ...
            data_modelos += `${n['id']}#${n['nome']}#${n['montadora']}#${n['valor_referencia']}#${n['motorizacao']}#${n['turbo']}#${n['automatico']}\n`
        }
    }

    fs.writeFileSync('modelos.txt', data_modelos)
}
*/
